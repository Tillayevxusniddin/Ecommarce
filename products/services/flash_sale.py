...
from products.models import Product, FlashSale, ProductViewHistory  # ProductViewHistory
...
def check_flash_sale(request, product_id):
    ...
    user_viewed = ProductViewHistory.objects.filter(user=request.user, product=product).exists()  # => True yoki false ya'ni ko'rgan bu mahsulotni yoki yo'q
    ...
    if user_viewed and upcoming_flash_sale: #agar foydalanuvchi buni korgan bolsa va yaqin 24 soat ichida skidka boshladigan bolsa
